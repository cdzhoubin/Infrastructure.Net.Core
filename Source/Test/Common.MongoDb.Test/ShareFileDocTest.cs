...
namespace Zhoubin.Infrastructure.Common.MongoDb.Test
{
    ...
    /// </summary>
    [TestClass]
    public class ShareFileDocTest : TestBase
    {
        ...
        {
            return Factory.CreateFileStorage("ShareFileDoc");
        }
        ...
        {
            var service = CreateFileStorage();
            var id = service.Insert(new MemoryStream(Resource1.File1), new FileEntity { FileName = "test",Database = "d"+DateTime.Now.ToString("yyyyMMdd")});
            Assert.AreEqual(false, id == ObjectId.Empty);
        }
        ...
        public void TestGet_1()
        {
            ...
            Assert.AreEqual(Resource1.File1.Length, entity.FileSize);
            Assert.AreEqual("test", entity.FileName);
            var stream = service.DownLoad<FileEntity>(new Dictionary<string, object>() { {"Id",id} });
        ...
        {
            var service = CreateFileStorage();
            var id = service.Insert(new MemoryStream(Resource1.File1), new FileEntity { FileName = "test", Database = "testdata" });
            Assert.AreEqual(false, id == ObjectId.Empty);
            var stream = service.DownLoad<FileEntity>(id);
            Assert.AreEqual(Resource1.File1.LongLength, stream.Length);
        }
